...
class BLIPExtractor(BaseExtractor):
    def __init__(
        ...
        time_setup["setup_blip"] = time.time() - time_start
        ...
        time_setup['init'] = time.time() - time_start
        ...
        time_setup["last_linear_layer"] = time.time() - time_start
    ...
    def forward(self, x):
        ...
        self.time_forwards[-1]["set_model_device"] = time.time() - time_start
        start_time = time.time()

        if not self.extract_features_flag:
            out = x["image"]
        ...
        if self.use_masked_patch_wise_feature:
            ...
            if self.freeze_blip:
                ...
                raw_out = self.extract_features(x)
            self.time_forwards[-1]["extract_masked_features"] = time.time() - time_start
            time_start = time.time()
            ...
            self.time_forwards[-1]["get_masked_features_post"] = time.time() - time_start
        ...
        self.time_forwards[-1]["extract_features"] = time.time() - start_time
        return self.fc_layer(out)
    
    def fc_layer(self, out):
        start_time = time.time()
        if self.last_linear_layer is not None:
            out = self.last_linear_layer(out)
        self.time_forwards[-1]["last_linear_layer"] = time.time() - start_time
    ...
    def custom_extract_features(self, sample):
        ...
        self.time_forwards[-1]["image_embeds_frozen"] = time.time() - start_time
        ...
        self.time_forwards[-1]["query_tokens_pp"] = time.time() - start_time
        ...
        self.time_forwards[-1]["setup_masks"] = time.time() - start_time
        ...
        if not self.use_visual_encoder_only:
            ...
            self.time_forwards[-1]["text_tokenizer_masking"] = time.time() - start_time
            start_time = time.time()
            ...
            self.time_forwards[-1]["query_output_bert"] = time.time() - start_time
            start_time = time.time()
            ...
            self.time_forwards[-1]["multimodal_embeds"] = time.time() - start_time
